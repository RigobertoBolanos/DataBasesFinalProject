...
IS
BEGIN 
    UPDATE PRODCLI SET PRODCLI.codigoProducto = codigoProducto WHERE PRODCLI.codigoProducto = codigoProducto AND PRODCLI.cliente_cedulacliente = cedulaCliente;  
END pModificar;
...
CURSOR productoClienteBuscado IS 
    SELECT *
    FROM PRODCLI 
    WHERE PRODCLI.codigoProducto = codigoProducto AND PRODCLI.cliente_cedulacliente = cedulaCliente;
codigoProducto NUMBER(8);
cliente_cedulacliente VARCHAR2(20);
constantes_codigoconstante VARCHAR2(20);
BEGIN
    OPEN productoClienteBuscado;
    FETCH productoClienteBuscado INTO codigoProducto, cliente_cedulacliente, constantes_codigoconstante;
    CLOSE funcionarioBuscado;
    
    RETURN codigoProducto || ' ' || cliente_cedulacliente || ' ' || constantes_codigoconstante;
END fConsultar;
